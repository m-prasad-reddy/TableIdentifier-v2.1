...
class PatternManager:
    ...
    and updates patterns based on user feedback. Caches patterns and entity mappings in Redis
    ...
    Attributes:
        ...
        entity_mappings (Dict[str, List[str]]): Intent-to-column mappings (e.g., "name" -> ["first_name", "last_name"]).
    """
    ...
    def _get_default_entity_mappings(self) -> Dict[str, List[str]]:
        ...
        return {
            ...
            "details": ["name", "address", "city", "state", "zip_code", "store_name"],
        ...
        for schema in self.schema_dict['tables']:
            for table in self.schema_dict['tables'][schema]:
                ...
                if any(col in self.schema_dict['columns'][schema][table] for col in ["first_name", "last_name"]):
                    ...
                    patterns.append([{"LOWER": "customer"}, {"LOWER": "names"}])
    ...
    def match_pattern(self, query: str) -> List[str]:
        """
        Match query against patterns, entities, and foreign keys.
        ...
        try:
            ...
            for ent in doc.ents:
                ent_type = ent.label_
                if ent_type in ["DATE", "GPE", "CARDINAL"]:
                    for schema in self.schema_dict['columns']:
                        for table in self.schema_dict['columns'][schema]:
                            full_table = f"{schema}.{table}"
                            for col_name, col_info in self.schema_dict['columns'][schema][table].items():
                                ...
                                if ent_type == "GPE" and ("city" in col_name.lower() or "state" in col_name.lower()):
                                    matches.add(full_table)
                                if ent_type == "CARDINAL" and col_info['type'].lower() in ['int', 'numeric', 'decimal']:
                                    matches.add(full_table)
    ...
    def update_patterns(self, query: str, tables: List[str]) -> None:
        """
        Update patterns and entity mappings based on user feedback.

        Adds query keywords to patterns for confirmed tables, enhancing future matches.
        ...
        try:
            ...
            for table in tables:
                ...
                for keyword in keywords:
                    # Add single-keyword pattern
                    new_pattern = [{"LOWER": keyword}]
                    if new_pattern not in self.patterns[table]:
